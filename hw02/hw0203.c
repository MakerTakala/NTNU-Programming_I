...
int main(){
    ...
    printf("From: ");
    scanf("%ld/%ld/%ld %ld:%ld", &(startTime.year), &(startTime.month), &(startTime.date), &(startTime.hour), &(startTime.minute));
    errorInputDetect(startTime);
    printf("To: ");
    scanf("%ld/%ld/%ld %ld:%ld", &(endTime.year), &(endTime.month), &(endTime.date), &(endTime.hour), &(endTime.minute));
    errorInputDetect(endTime);
    errorTimeDetect(startTime, endTime);

    int64_t allDay = allDayCounter(startTime, endTime);
    int64_t workDay = workDayCounter(startTime, endTime, allDay);
...
int64_t allDayCounter(time start,  time end){
    int64_t y1, m1, d1;
    int64_t y2, m2, d2;
    m1 = (start.month + 9) % 12;
    y1 = start.year - m1 / 10;
...
int64_t workDayCounter(time start, time end, int64_t allDay){
    int8_t startWeek = weekCounter(start);
    int8_t endWeek = weekCounter(end);
    int64_t holidayDay = ((allDay - 1) / 7) * 2 ;
    ...
    return allDay - holidayDay - (endWeek == 6 || endWeek == 0);
}
